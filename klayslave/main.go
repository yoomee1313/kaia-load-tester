...
func prepareERC20Transfer(accGrp map[common.Address]*account.Account) {
	if !inTheTCList("erc20TransferTC") {
		return
	}
	...
	erc20TransferTcSca := deploySingleSmartContract(erc20DeployAcc, erc20DeployAcc.DeployERC20, "ERC20 Performance Test Contract")
	newCoinBaseAccountMap := map[common.Address]*account.Account{newCoinbase.GetAddress(): newCoinbase}
	firstChargeTokenToTestAccounts(newCoinBaseAccountMap, erc20TransferTcSca.GetAddress(), erc20DeployAcc.TransferERC20, big.NewInt(1e11))

	chargeTokenToTestAccounts(accGrp, erc20TransferTcSca.GetAddress(), newCoinbase.TransferERC20, big.NewInt(1e4))
...
func prepareERC721Transfer(accGrp []*account.Account) {
	if !inTheTCList("erc721TransferTC") {
		return
	}
	...
	erc721TransferTcSca := deploySingleSmartContract(erc721DeployAcc, erc721DeployAcc.DeployERC721, "ERC721 Performance Test Contract")

	// Wait for reward tester to get started
	time.Sleep(30 * time.Second)
	newCoinbase.MintERC721ToTestAccounts(gCli, accGrp, erc721TransferTcSca.GetAddress(), 5)
...
func prepareStorageTrieWritePerformance(accGrp map[common.Address]*account.Account) {
	if !inTheTCList("storageTrieWriteTC") {
		return
	}
	...
	storageTrieWriteTcSca := deploySingleSmartContract(storageTrieDeployAcc, storageTrieDeployAcc.DeployStorageTrieWrite, "Storage Trie Performance Test Contract")
...
func main() {
	...
	taskSet := task.InitTCList(accGrpForSignedTx, accGrpForUnsignedTx, gEndpoint)
